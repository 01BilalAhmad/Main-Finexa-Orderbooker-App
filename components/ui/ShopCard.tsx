...
export const ShopCard = memo(function ShopCard({
  ...
  // Memoize balance calculations to avoid recomputation on every render
  const { balance: displayBalance, creditLimit: displayCreditLimit } = useMemo(
    () => getShopDisplayBalance(shop, companyId),
    [shop, companyId]
  );
  const isOverLimit = useMemo(() => displayBalance > displayCreditLimit, [displayBalance, displayCreditLimit]);
  const rawUtilisation = useMemo(() => displayCreditLimit > 0 ? (displayBalance / displayCreditLimit) * 100 : 0, [displayBalance, displayCreditLimit]);
  const utilisation = useMemo(() => Math.min(rawUtilisation, 100), [rawUtilisation]);
  const isApproachingLimit = useMemo(() => !isOverLimit && rawUtilisation >= 90, [isOverLimit, rawUtilisation]);
  const isZeroBalance = useMemo(() => displayBalance === 0, [displayBalance]);
  const barColor = useMemo(() => isOverLimit ? Colors.danger : rawUtilisation >= 90 ? Colors.secondary : utilisation > 80 ? Colors.secondary : Colors.primary, [isOverLimit, rawUtilisation, utilisation]);
