...
export const PerformanceChart = memo(function PerformanceChart({ userId }: PerformanceChartProps) {
  ...
  const chartValues = useMemo(() => data.map((d) => Math.max(d.amount, 0)), [data]);
  const hasData = useMemo(() => chartValues.some((v) => v > 0), [chartValues]);
  const chartLabels = useMemo(() => data.map((d) => d.label), [data]);
  const maxVal = useMemo(() => Math.max(...chartValues, 1), [chartValues]);

  const trendIcon = useMemo(() => trend === 'up' ? 'trending-up' : trend === 'down' ? 'trending-down' : 'trending-flat', [trend]);
  const trendColor = useMemo(() => trend === 'up' ? '#A7F3D0' : trend === 'down' ? '#FCA5A5' : 'rgba(255,255,255,0.7)', [trend]);
      ...
      <View style={styles.chartBody}>
        {loading ? (
          <View style={styles.loadingBox}>
            {/* Skeleton placeholders while data is being fetched */}
