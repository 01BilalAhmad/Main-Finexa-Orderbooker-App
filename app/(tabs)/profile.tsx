...
export default function ProfileScreen() {
  ...
  const handleSavePhone = async () => {
    ...
    if (trimmed && !/^0\d{10}$|^0\d{3}-?\d{7}$/.test(trimmed)) {
      Alert.alert('Invalid Phone', 'Please enter a valid Pakistani phone number (e.g., 03XXXXXXXXX)');
      return;
    }
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.themeSection}>
          ...
          <View style={styles.themeRow}>
            {([
              { key: 'light' as ThemeMode, label: 'Light', icon: 'light-mode' as const },
              { key: 'dark' as ThemeMode, label: 'Dark', icon: 'dark-mode' as const },
              { key: 'system' as ThemeMode, label: 'System', icon: 'settings' as const },
            ]).map((option) => (
              <Pressable
                key={option.key}
                style={({ pressed }) => [
              ...
              >
                ...
                <Text style={[
                  ...
                  themeMode === option.key && styles.themeOptionLabelActive,
                ]}>
                  {option.label}
                </Text>
              </Pressable>
