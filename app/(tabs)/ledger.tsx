...
export default function LedgerScreen() {
  ...
  const filteredShops = shopSearch.trim()
    ? allShops.filter(
        (s) =>
          ...
          (s.phone && s.phone.toLowerCase().includes(shopSearch.toLowerCase()))
      )
    : allShops;
  ...
  const filteredTxns = React.useMemo(() => {
    if (!isDateFilterActive) return reversedTxns;
    return reversedTxns.filter((t) => {
      const txnDate = t.createdAt.slice(0, 10); // YYYY-MM-DD
      if (dateFrom && txnDate < dateFrom) return false;
      if (dateTo && txnDate > dateTo) return false;
      return true;
    });
  }, [reversedTxns, dateFrom, dateTo, isDateFilterActive]);
            ...
            <View>
              ...
              <View style={styles.txnHeader}>
                ...
                <View style={styles.txnCountBadge}>
                  <Text style={styles.txnCountText}>{isDateFilterActive ? filteredTxns.length : ledger.summary.totalTransactions}</Text>
                </View>
              </View>
              ...
              <View style={styles.dateFilterCard}>
                <View style={styles.dateFilterRow}>
                  <View style={styles.dateFilterField}>
                    ...
                    <TextInput
                      style={[styles.dateFilterInput, dateFrom && styles.dateFilterInputActive]}
                      value={dateFrom}
                  ...
                  <MaterialIcons name="arrow-forward" size={16} color={Colors.textMuted} style={{ marginTop: 16 }} />
                  <View style={styles.dateFilterField}>
                    <Text style={styles.dateFilterLabel}>To</Text>
                    <TextInput
                      style={[styles.dateFilterInput, dateTo && styles.dateFilterInputActive]}
                      value={dateTo}
                ...
                {isDateFilterActive ? (
                  <View style={styles.dateFilterActiveRow}>
                    <Text style={styles.dateFilterActiveText}>
                      Filtered: {filteredTxns.length} transaction{filteredTxns.length !== 1 ? 's' : ''}
                    </Text>
                    <Pressable onPress={clearDateFilter} style={styles.dateFilterClearBtn}>
        ...
        <View style={styles.pickerSheet}>
          ...
          <View style={styles.pickerSearch}>
            ...
            <TextInput
              ...
              placeholder="Search shops, phones..."
              placeholderTextColor={Colors.textMuted}
              autoFocus
            />
          </View>
