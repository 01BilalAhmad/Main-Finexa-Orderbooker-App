...
import { useRecoverySubmission } from '@/hooks/useRecoverySubmission';
...
export default function TodayRouteScreen() {
  ...
  // ── Custom hooks ─────────────────────────────────────────────────────────
  // 1. Visit tracking (no cross-hook deps)
  const { visitedShopIds, handleGpsVisitMarked, setVisitedShopIds } = useVisitTracking({ user, isOnline });

  // 2. Today stats (needs setVisitedShopIds for API merge)
  const {
    todayRecovery, setTodayRecovery, loadTodayStats,
    smsSentCount, setSmsSentCount,
    whatsappSentCount, setWhatsappSentCount,
  } = useTodayStats({ user, setVisitedShopIds });

  // 3. Notification flow (needs lastRecoveryInfoRef for handlePhoneSaved)
  const lastRecoveryInfoRef = useRef<import('@/hooks/useNotificationFlow').LastRecoveryInfo | null>(null);
  const {
    pendingNotifAfterSuccess, setPendingNotifAfterSuccess,
    notifChoice, setNotifChoice,
    phoneInputShop, setPhoneInputShop,
    handlePhoneSaved,
    pendingNotifications, loadPendingNotifications,
    showPending, setShowPending,
  } = useNotificationFlow({ user, distributorPhone, lastRecoveryInfoRef, recoveryShop });

  // 4. Recovery submission (needs notification setters for cross-hook communication)
  const {
    handleSubmitRecovery, handleUndoRecovery, handleOpenRecovery,
    isSubmitting, successState, setSuccessState,
    lastRecoveryInfo, setLastRecoveryInfo,
    recoverySubmittedShopIds,
  } = useRecoverySubmission({
    user, isOnline, recoveryShop, distributorPhone, addToOfflineQueue,
    setRecoveryShop, setTodayRecovery, setVisitedShopIds,
    setPendingNotifAfterSuccess, setNotifChoice, setPhoneInputShop,
    loadPendingNotifications,
  });

  // Keep the ref in sync so useNotificationFlow can read latest lastRecoveryInfo
  lastRecoveryInfoRef.current = lastRecoveryInfo;
  ...
  const groupedSections = useMemo(() => {
    ...
    for (const shop of filteredShops) {
      if (assignedShops.has(shop.id)) continue;
    ...
    for (const dayKey of sortedDays) {
      ...
      items.push({ type: 'header', dayKey, dayLabel: DAY_LABELS[dayKey] || capitalize(dayKey), shopCount: dayShops.length, isToday });
      ...
      <ShopDetailModal
        ...
        onResendReceipt={() => {
          ...
          Linking.openURL(whatsappUrl).then(() => {
            Alert.alert(
              'WhatsApp Opened',
              `WhatsApp chat opened for ${detailShop.name}.\n\nTap attachment → Gallery → "Finexa Receipts" → Select receipt → Send`,
              [{ text: 'OK' }]
            );
          }).catch(() => {
...
// ── Hero badge label (computed inline above) ─────────────────────────────
